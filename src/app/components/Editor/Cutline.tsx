...
interface CutlineProps {
  expandCutLine: React.Dispatch<React.SetStateAction<string | null>>;
}
...
export default function Cutline({ expandCutLine }: CutlineProps) {
  ...
  const options: React.ReactNode[] = Object.keys(cutlineSettings).map(
    (el: string) => {
      return (
        <div
          key={el}
          className={`hover:bg-slate-200 rounded hover:cursor-pointer ${
            activeIcon === el ? "bg-gray-300 py-2" : "hover:bg-gray-300 py-2"
          }}`}
          onClick={() => {
            handleIconClick(el);
          }}
  ...
  return (
    <div className="outline-tool-container flex justify-center self-start bg-slate-100 h-1/6 w-full">
      <div>
        ...
        <div className="grid grid-cols-3 gap-4">{options}</div>
      </div>
    </div>
  );
}
